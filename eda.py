...
# EDA 2-1)
# 프로모션 적용 여부에 따른 평균 수요 차이가 통계적으로 유의한지 T-test 검증
# =========================================================
from scipy.stats import ttest_ind

promo_demand = tmp[tmp["promotion_applied"] == 1]["quantity_sold"]
no_promo_demand = tmp[tmp["promotion_applied"] == 0]["quantity_sold"]
...
ttest_result = pd.DataFrame({
    ...
    "value": [round(t_stat, 3), round(p_value, 5)]
})
...
# EDA 4-1) Forecast Error vs Reorder Signal Correlation
# 예측 오차가 재주문 신호 발생과 연관되어 있는지 검증
# =========================================================

err_corr_df = df[["abs_demand_error", "needs_reorder"]].dropna()

spearman_corr = err_corr_df.corr(method="spearman")
...
# =========================================================
# EDA 6-1) t-test검증
# 재주문 신호 여부에 따른 평균 수요 차이 검증
# =========================================================

reorder_yes = df[df["needs_reorder"] == 1]["quantity_sold"]
reorder_no = df[df["needs_reorder"] == 0]["quantity_sold"]
...
reorder_ttest = pd.DataFrame({
    "metric": ["t_statistic", "p_value"],
    "value": [round(t_stat_r, 3), round(p_value_r, 5)]
})

display(reorder_ttest)
